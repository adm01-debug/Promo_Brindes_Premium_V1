...
| Comprador repete o envio | Duas oportunidades ou duas propostas | Chave de idempotência, limite de tentativas, protocolo após resposta do receptor | Persistência transacional compartilhada e teste de retry no CRM |
...
| Múltiplas réplicas ou reinicialização | Cache de idempotência perdido | Limite documentado; nenhum lançamento é permitido com Map em memória | Tabela/fila aprovada, reconciliação e rollback exercitado |
...
3. Escolher receptor e armazenamento idempotente; integrar e exercitar retries em staging.
