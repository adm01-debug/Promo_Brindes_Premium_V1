...
| MVP público  | `/catalogos` e `/catalogos/[slug]`   | Seis coleções editoriais por ocasião                       | Bloqueadas enquanto a prévia não for liberada           |
| Futuro       | guias e páginas institucionais       | Exigem conteúdo, critérios e mídia aprovados               | Não criado                                              |

As facetas de descoberta são estado da vitrine, não novas páginas editoriais. A URL usa `q`, `categoria`, `ocasiao`, `personalizavel`, `quantidade`, `page` e `sort`; o canonical da home permanece `/`. Isso evita transformar combinações de filtros em duplicatas indexáveis.

## Taxonomia atual

As únicas categorias publicadas são `Kits & experiências`, `Escrita`, `Lifestyle` e `Viagem`. Cada card preserva seu mesmo ID e SKU quando muda de filtro. As seis coleções editoriais publicadas definem elegibilidade por ocasião usando IDs canônicos. Essa relação expressa curadoria e não afirma prazo, técnica, preço, estoque ou disponibilidade.

## Jornada sem conta e retorno

1. A pessoa explora a curadoria, busca por nome, categoria, SKU ou contexto editorial, combina filtros verificáveis e pode favoritar itens.
...
| Painel de filtros     | Estado controlado, aplicação imediata, contagens contextuais, chips removíveis e URL restaurável. |
