...
**Revisão posterior:** os cenários sintéticos confirmam limite de corpo real, rejeição de data inválida, hash de payload, paginação de fonte e uma única entrega concorrente por chave. O POST consulta o catálogo sem cache; o contrato rejeita drift de campos, duplicatas e ficha com slug divergente. Uma confirmação local perdida após aceite do receptor permanece incerta, sem marcação falsa de falha. A persistência usa chave única, hash e lease no banco dedicado. A RLS do banco dedicado da vitrine foi verificada em `audit/plan-database-check.json`; a auditoria administrativa do banco operacional e o fluxo de acesso entre clientes permanecem abertos.
...
| Publicação retirada | Briefing aceitar produto já despublicado por cache da fonte | Conferência sem cache no navegador e no POST; falha da fonte bloqueia a operação | Backend + editorial |
| Drift editorial | Slug, data, mídia ou paginação inválidos chegarem à vitrine | Validação de projeção, contagem, unicidade e correspondência do slug | Dados + backend |
...
| Aceite remoto sem confirmação local | Reenvio de oportunidade já aceita após expirar a reserva | Não registrar falha falsa; exigir idempotência e conciliação pelo protocolo no CRM | Backend + operações |
