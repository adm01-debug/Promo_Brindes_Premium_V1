...
| Push e pull request | Gates dependiam de execução manual | Workflow versionado executa instalação limpa, tipos, plano, build, scanner, orçamento, cenários e matriz Chromium/Firefox/WebKit | `.github/workflows/quality.yml`; os runs anteriores de Chromium passaram e a matriz tripla passou localmente. |
| Home sem código de planejamento | O visitante poderia baixar ferramentas internas junto da vitrine | Chunks exclusivos de `/planejamento`; gate rejeita marcador do painel nos chunks iniciais da home | `scripts/check-performance-budget.mjs` sobre o build de produção |
