...
| Push e pull request | Gates dependiam de execução manual | Workflow versionado executa instalação limpa, tipos, plano, build, scanner, orçamento, cenários e Chromium | `.github/workflows/quality.yml`; `actionlint` local passou. Execução remota deve ser conferida no GitHub após publicação. |
